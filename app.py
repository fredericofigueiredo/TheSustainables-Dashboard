# -*- coding: utf-8 -*-
"""
Created on Sat Oct  9 14:58:01 2021
...
#                                                       'total_visitors'])
    ...
    def get_country(df, worldcities):
        
        cities = df['City']
        countries = []
        
        for city in cities:
            if city in list(worldcities['city']):
                countries.append(list(worldcities[worldcities['city']==city]['country'])[0])
            else:
    ...
    url = 'https://raw.githubusercontent.com/fredericofigueiredo/TheSustainables-Dashboard/main/worldcities.csv'
    
    download = requests.get(url).content
    
    worldcities = pd.read_csv(io.StringIO(download.decode('utf-8')), error_bad_lines = False)
    ...
    def product_tagging(df):
        ...
        sku=[]
        df['Product Name'] = list(df['Product Name'].apply(str))
    ...
    if avocadostore_upload is not None:
        ...
        get_country(avocado, worldcities)
     ...
     for i, row in df.iterrows():
         ...
         if len(row['SKU'])>0:
             ...
             if row['Channel']=="Shopify":
                 ...
                 if "3er" in row['Product Name']:
                     if len(row['SKU']) == 1:
                         total.extend([row['SKU'][0],row['SKU'][0],row['SKU'][0]])
                     elif len(row['SKU']) == 2:
                         total.extend([row['SKU'][0],row['SKU'][0],row['SKU'][1]])
                     elif len(row['SKU']) == 3:
                         total.extend([row['SKU'][1],row['SKU'][0],row['SKU'][2]])
                     else:
                         continue
                     
                 if "6er" in row['Product Name']:
                     if len(row['SKU']) == 1:
                         ...
                         total.extend([row['SKU'][0],row['SKU'][0],row['SKU'][1],row['SKU'][1],row['SKU'][2],row['SKU'][2]])
                     else:
                         continue
                     
                 if "9er" in row['Product Name']:
                     if len(row['SKU']) == 1:
    ...
    #LAYOUT    
        ...
        l2c1.metric("Value of Returns", str(round(n_returns,2))+"€")
